...
const weaponState = [
  { name_: '', ammo_: null, lastShotDate_: -Infinity, type_: '' },
  { name_: '', ammo_: null, lastShotDate_: -Infinity, type_: '' },
  { name_: '', ammo_: null, type_: '', lastShotDate_: -Infinity },
  { name_: '', ammo_: null, type_: '', lastShotDate_: -Infinity },
];
...
const isSlowFiringWeapon = (weaponType) => {
  try {
    ...
    return (
      (weapon.fireMode === 'single' || weapon.fireMode === 'burst') && weapon.fireDelay >= 0.35
    );
  } catch {
    return false;
  }
...
const handleWeaponSwitch = () => {
  ...
  try {
    ...
    const timeSinceLastSwitch = Date.now() - currentWeaponState.lastShotDate_;
    if (timeSinceLastSwitch < settings.autoSwitch_.switchCooldown_) return;
