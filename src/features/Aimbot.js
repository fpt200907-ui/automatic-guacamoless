...
import { updateMeleeLock, setMeleeLockPreviousEnemies, resetMeleeLockState, getMeleeLockEnemy } from '@/features/MeleeLock.js';
import {
  AimState,
  setAimState,
...
function aimbotTicker() {
  try {
    ...
    try {
      ...
      // Share previousEnemies_ with MeleeLock for velocity history
      setMeleeLockPreviousEnemies(state.previousEnemies_);
      
      // Update melee lock
      const meleeLockResult = updateMeleeLock(me, players, currentWeaponIndex, isAiming, canCastToPlayer);
      if (meleeLockResult) {
        aimUpdated = true;
        aimOverlays.hideAll();
        state.lastTargetScreenPos_ = null;
      ...
      if (enemy) {
        ...
        if (!predictedPos || !Number.isFinite(predictedPos.x) || !Number.isFinite(predictedPos.y)) {
          setAimState(new AimState('idle'));
          aimOverlays.hideAll();
          state.lastTargetScreenPos_ = null;
