...
const applyAutoFire = (packet) => {
  if (!autoFireEnabled && !autoAttackFireEnabled) return;
  
  const activePlayer = gameManager.game?.[translations.activePlayer_];
  
  // Don't auto-attack fire while reloading or healing (manual aim can still work)
  if (autoAttackFireEnabled && (isWeaponReloading(activePlayer) || isPlayerHealing(activePlayer))) {
    return;
  }
